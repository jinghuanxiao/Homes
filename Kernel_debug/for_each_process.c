...
static int hello_init(void) {
    ...
    for_each_process(task) {
        ...
        switch (task->state) {
        case TASK_RUNNING:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_RUNNING");
            break;
        ...
        case TASK_UNINTERRUPTIBLE:
            ...
            break;
        case __TASK_STOPPED:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "__TASK_STOPPED");
            break;
        case __TASK_TRACED:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "__TASK_TRACED");
            break;
        case EXIT_DEAD:
        ...
        case TASK_NOLOAD:
            ...
            break;
        case TASK_NEW:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_NEW");
            break;
        case TASK_STATE_MAX:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_STATE_MAX");
            break;
        case TASK_KILLABLE:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_KILLABLE");
            break;
        case TASK_STOPPED:
        ...
        case TASK_TRACED:
            ...
            break;
        case TASK_IDLE:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_IDLE");
            break;
        case TASK_NORMAL:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_NORMAL");
            break;
        case TASK_ALL:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_ALL");
            break;
        case TASK_REPORT:
            printk(KERN_INFO "Process ID: %d Name: %-20s State: %s\t\n", task->pid, task->comm, "TASK_REPORT");
            break;
        default :
            printk(KERN_WARNING "error!, %ld\n", task->state);
        }
    }
