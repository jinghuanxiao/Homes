...
Curl_hash_init(struct curl_hash *h,
               ...
               curl_hash_dtor dtor)
{
  if(!nslots || !hfunc || !comparator ||!dtor) {
    return 1; /* failure */
  }
  ...
  h->table = malloc(nslots * sizeof(struct curl_llist));
  if(h->table) {
    int i;
    for(i = 0; i < nslots; ++i)
...
#define FETCH_LIST(x,y,z) &x->table[x->hash_func(y, z, x->nslots)]
...
Curl_hash_next_element(struct curl_hash_iterator *iter)
{
  ...
  if(!iter->current_element) {
    int i;
    for(i = iter->slot_index; i < h->nslots; i++) {
      if(h->table[i].head) {
